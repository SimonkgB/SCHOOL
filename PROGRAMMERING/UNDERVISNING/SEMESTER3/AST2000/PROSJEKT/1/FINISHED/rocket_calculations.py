...
def thrust(escaped_velocities, hydrogen2_mass, totaltime):
    ...
    avg_velocity =np.mean(escaped_velocities, axis=0)
...
def n_motors(thrust, escape_velocity, time, rocket_mass):
...
def massflow(thrust, avg_velocity):
    return np.abs(thrust/avg_velocity) # for n particles this will stay constant
...
total_thrust_2 = thrust(w.escaped_velocities, cs.hydrogen2_mass, cs.totaltime)*n_motors(total_thrust, escape_velocity, cs.time_to_v_esc, cs.rocket_payload)
