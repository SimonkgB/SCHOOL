...
i = 0
for _ in range(n-1):
    M[i*2] = np.array([(m1+m2)*l1**2, m2*l1*l2*np.cos(phi[0][i]-phi[1][i])])
    M[i*2+1] = np.array([m2*l1*l2*np.cos(phi[0][i]-phi[1][i]), m2*l1**2])
    ml = np.array([M[2*i],M[2*i+1]])
